...
const long long MAX_LENGTH = 100000;
const char *KEY_PATH = "key.txt";
const char *SOURCE_TEXT = "text.txt";
const char *ENCRYPTED_TEXT = "enc_text.txt";
...
bool decode(const char* path, const int* key_val, int count_of_key) {
	...
	ofstream out("decoded.txt", ios::out);
	if (!out.is_open()) {
		cout << "Cannot open for reading file: decoded.out\n";
		return false;
	}

	int c = 0; long long i = 0;
	for (i = 0; (c = in.get()) != -1 ; ++i) {
